...
class Utils {
  ...
  String getTimeDifference(DateTime dateTime) {
    return getTimeText(DateTime.now().difference(dateTime).inMilliseconds);
  }

  /// Formats elapsed VPN/core uptime for the start button.
  ///
  /// Under 24 hours: `HH:MM:SS`.
  /// 24 hours and above: `Nd HH:MM:SS` so multi-day runs stay readable and are
  /// not clipped by the old `999:59:59` / two-digit-hour layout ceiling.
  String getTimeText(int? timeStamp) {
    ...
    final hours = (totalSeconds % Duration.secondsPerDay) ~/
        Duration.secondsPerHour;
    final minutes = (totalSeconds % Duration.secondsPerHour) ~/
