...
class _CategoryChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (loading && delay == null) {
      ...
      color = utils.getDelayColor(delay) ?? context.colorScheme.primary;
      trailing = '${delay}ms';
    } else if (hasFailure) {
      color = Colors.red;
      trailing = '×';
...
class ServiceReachabilitySheet extends ConsumerWidget {
  ...
  Future<void> _handleEnableAuto(BuildContext context, WidgetRef ref) async {
    ...
    final result = await enableAutoSelectWithContainer(globalState.container);
    if (!context.mounted) {
      return;
    }
    final message = switch (result.message) {
      ...
      _ when result.enabledExisting =>
        l10n.enableAutoSelectRestored(result.groupName ?? ''),
      _ when result.createdGroups =>
        l10n.enableAutoSelectCreated(result.groupName ?? ''),
            ...
            ListTile(
              ...
              trailing: Text(
                ...
                style: context.textTheme.bodyMedium?.copyWith(
                  color: utils.getDelayColor(state.results[probe.id]),
                ),
              ),
            ),
          const SizedBox(height: 12),
